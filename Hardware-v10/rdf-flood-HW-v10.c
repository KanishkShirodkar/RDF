...
/* =========================================================================
 * Board name lookup — converts MAC-derived node_id to human-readable
 * "m3-XX" string for easy manual log reading.
 * Add a new entry for every node you deploy.
 * Returns "m3-??" if the node_id is not in the table.
 * =========================================================================
 */
               ...
               uint16_t arriving_hop, const char *phase_label)
{
  ...
  if(phase_label[0] == 'c') {
    LOG_INFO("CBF_HOP_IGNORE src=%s seq=%u"
             " arriving_hop=%u pending_hop=%u\n",
             board_name(src), seq,
             (unsigned)arriving_hop,
             (unsigned)st->pending_msg.hop_count);
  } else {
    LOG_INFO("RDF_HOP_IGNORE src=%s seq=%u"
             " arriving_hop=%u pending_hop=%u period=%s\n",
             board_name(src), seq,
             (unsigned)arriving_hop,
             (unsigned)st->pending_msg.hop_count,
                            ...
                            flood_msg_t *msg, const char *phase_label)
{
  ...
  LOG_INFO("PENDING_DROP_STALE src=%s seq=%u buffered_seq=%u stale_seq=%u"
           " period=%s\n",
           board_name(src), seq,
...
start_cbf_phase(uint16_t src, flood_msg_t *msg, uint16_t slot)
{
  ...
  LOG_INFO("CBF_TIMER src=%s seq=%u hop=%u sender_xy=(%d,%d) "
...
{
  LOG_INFO("DISSEM src=%s seq=%u node=%s recv_by_me=%" PRIu32
           " last_seq=%u\n",
           board_name(src), seq, board_name(node_id), recv_count[slot], last_seq_seen[slot]);
}
...
log_stats_summary(void)
{
  ...
  for(slot = 0; slot < MAX_NODES; slot++) {
    ...
    LOG_INFO("STATS node=%s src=%s recv=%" PRIu32 " exp=%" PRIu32
             " fwd=%" PRIu32 "\n",
             board_name(node_id), board_name(slot_to_srcid[slot]),
             recv_count[slot],
                ...
                uint16_t datalen)
{
  ...
  LOG_INFO("RX src=%s seq=%u hop=%u src_xy=(%d,%d) sender_xy=(%d,%d) "
           "my_xy=(%d,%d)\n",
           board_name(src), seq, msg.hop_count,
...
PROCESS_THREAD(rdf_process, ev, data)
{
  ...
  LOG_INFO("START node=%s send_interval_ticks=%lu\n",
